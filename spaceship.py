...
class Ship:
    ...
    def update(self):
        ...
        if self.thrust == True:
            ...
            self.vel[0] = forward[0] * key
            self.vel[1] = forward[1] * key
    ...
    def angvel_increase(self):
        self.angle_vel += .04
         
    def angvel_decrease(self):
        self.angle_vel -= .04
...
def down(key):
    if key == simplegui.KEY_MAP['left']:
        my_ship.angvel_decrease()
    if key == simplegui.KEY_MAP['right']:
        my_ship.angvel_increase()
...
def rock_spawner():
    ...
    [random.choice([0.5, -1]) * random.random() * 2, random.choice([0.5, -1]) * random.random() * 2], 0,
    random.choice([0.5, -1]) * 0.1, asteroid_image, asteroid_info)
