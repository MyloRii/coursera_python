...
class Ship:
    ...
    def update(self):
        ...
        # friction
        friction = 0.01
        self.vel[0] *= 1 - friction #.99
        self.vel[1] *= 1 - friction #.99
    ...
    def shoot(self):
        ...
        #version from coursera
        missile_pos = [self.pos[0] + self.radius * forward[0], self.pos[1] + self.radius * forward[1]]
        missile_vel = [self.vel[0] + 6 * forward[0], self.vel[1] + 6 * forward[1]]
        #my version
        #missile_pos = [self.pos[0] + self.radius * forward[0], self.pos[1] + self.radius * forward[1]]
        #missile_vel = [self.vel[0] + forward[0] * 2, self.vel[1] + forward[1] * 2]
...
#    canvas.draw_text(str(score), [680, 80], 22, "White")
    ...
    #colision between rocks & missiles 
...
def rock_spawner():
    ...
    # spawn rocks at 100px away from the ship
...
def group_group_collide(group, other_group):
    collisions_num = 0
    for object in set(group):
        collision = group_collide(other_group, object)
        if collision:
            group.remove(object)
            collisions_num += 1
    return collisions_num
...
rock_group = set() #([WIDTH / 3, HEIGHT / 3], [1, 1], 0, .1, asteroid_image, asteroid_info)
